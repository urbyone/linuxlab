...
resource "azurerm_network_interface" "nic1" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${azurerm_resource_group.rsg1.name}/providers/Microsoft.Network/publicIPAddresses/${azurerm_public_ip.pip1.name}"
...
resource "azurerm_linux_virtual_machine" "azvm1" {
  ...
  location              = azurerm_resource_group.rsg1.location
  name                  = var.vmname
  network_interface_ids = ["/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${azurerm_resource_group.rsg1.name}/providers/Microsoft.Network/networkInterfaces/${azurerm_network_interface.nic1.name}"]
  resource_group_name   = azurerm_resource_group.rsg1.name
  size                  = var.vmSKU
...
resource "azurerm_network_interface_security_group_association" "nsgadd1" {
  network_interface_id      = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${azurerm_resource_group.rsg1.name}/providers/Microsoft.Network/networkInterfaces/${azurerm_network_interface.nic1.name}"
...
resource "azurerm_monitor_activity_log_alert" "log_activity" {
  ...
  resource_group_name = var.rsgname
  location            = "global"
  scopes              = ["/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${var.rsgname}"]
  action {
    action_group_id = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourcegroups/${var.rsgname}/providers/microsoft.insights/actiongroups/${azurerm_monitor_action_group.amag.name}"
  }
  criteria {
...
resource "azurerm_virtual_machine_extension" "ama" {
  ...
  virtual_machine_id         = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${var.rsgname}/providers/Microsoft.Compute/virtualMachines/${var.vmname}"
  depends_on = [
    azurerm_linux_virtual_machine.azvm1
  ]
}
...
resource "azurerm_monitor_data_collection_rule" "res-5" {
  ...
  destinations {
    log_analytics {
      ...
      workspace_resource_id = "/subscriptions/${data.azurerm_subscription.current.subscription_id}/resourceGroups/${var.rsgname}/providers/Microsoft.OperationalInsights/workspaces/${var.rsgname}-log"
    }
  }
...
resource "azurerm_role_assignment" "rbac-user-blob" {
  ...
  scope                = azurerm_storage_account.sto.id
...
resource "azurerm_role_assignment" "rbac-user-priv" {
  principal_id         = var.userid
  scope                = azurerm_storage_account.sto.id
